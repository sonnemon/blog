...
import { Form, Checkbox, Segment, Message, Button, Image } from 'semantic-ui-react';
...
export default function FormPost(props) {
	...
	const [ formValues, setFormValues ] = useState(() => initialState(defaultValues));
	...
	return (
		<Segment loading={loading}>
			<Form onSubmit={(e) => e.preventDefault()} encType={'multipart/form-data'}>
							...
							<Image
								src={
									hasEdited ? (
										...
										`${process.env.API}${formValues.coverImage}`
									)
								}
								size="medium"
								centered
				...
				<Form.Button fluid color="blue" onClick={handleSubmitForm} content="Send" />
				{errors.length > 0 && (
					<Message color="red">
