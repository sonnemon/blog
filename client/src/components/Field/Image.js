...
import { Form, Button, Header, Image, Checkbox, Segment } from 'semantic-ui-react';
				...
				<Form onSubmit={(_e) => _e.preventDefault()}>
					<Form.Field>
						<Segment textAlign="center">
							<Segment basic>
								<Image
									src={hasEdited ? data.value : `${process.env.API}${data.value}`}
									size="medium"
									centered
								/>
							</Segment>
					...
					<Form.Group widths="equal">
						...
						<Form.Field>
							...
							<Checkbox
								...
								onChange={(_e, { checked }) => handleChange('isCentered', checked)}
							/>
						</Form.Field>
					</Form.Group>
...
const Present = ({ data }) => {
	return (
		<Image
			...
			src={`${process.env.API}/${data.value}`}
			size={data.size}
		/>
	);
};
